...
# -----------------------------
def build_metric_lookup(json_file_path='bspt_performance_json_file.json'):
    """
...
# dump. Business Partner has 336 metrics (vs ESGRC's 84) and the correlation
# matrix dump and inconsistency table both scale with the SQUARE of the metric
# count, so the untrimmed outputs are far larger. Customer (348 metrics) hit this
# first on 2026-07-31: a 9.1MB combined step-6 report that overflowed Claude's
# context window (guard.py's fallback char/token estimate also undercounted the
# dense tabular content - see guard.py's CHARS_PER_TOKEN_FALLBACK comment for
# that half of the fix). Applied here preemptively rather than on a live 400. This
...
def main(metrics_csv_file_path, group_csv_file_path, sub_module_csv_file_path):
    ...
    json_path = 'bspt_performance_json_file.json'  # Adjust path if needed
    ...
    # File names
    output_report_file = 'M_G_SM_correlation_report_bspt.txt'
    trends_file = 'trends_and_repetitions_report_bspt.txt'
    inconsistencies_file = 'inconsistencies_report_bspt.txt'
    chaid_report_file = 'chaid_risk_segmentation_report_bspt.txt'
...
if __name__ == "__main__":
    metrics_csv_file_path = 'filtered_metrics_data_bspt.csv'
    group_csv_file_path = 'filtered_groups_data_bspt.csv'
    sub_module_csv_file_path = 'filtered_sub_modules_data_bspt.csv'
