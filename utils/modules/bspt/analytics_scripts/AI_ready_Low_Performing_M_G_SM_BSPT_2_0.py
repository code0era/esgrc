...
metrics_file = 'input_metric_values_bspt.csv'
json_file = 'bspt_performance_json_file.json'
...
def save_averages_to_csv(metric_data, group_averages, sub_module_averages, module_averages, output_csv_file):
    ...
    if module_id:
        ...
        new_columns['BSPT_001'] = module_avg_series
    ...
    print("Output file created with consolidated values and BSPT_001 column.")

# Define the output CSV file name
output_csv_file = 'module_values_bspt.csv'
...
dependent_variable = 'BSPT_001'
if dependent_variable not in metric_data.columns:
    raise KeyError(f"'{dependent_variable}' not found in the DataFrame columns.")

# Ranking aggregation. This previously read .iloc[0] - a single arbitrary row out
# of ~3000 observations, with no date/period column in the input to make row 0
# meaningful. The lowest performers were therefore chosen from one unspecified
# observation rather than the series. Mean across the series is the defensible
# default; change AGGREGATOR if the analytics owner wants median or latest.
# Define function to identify low-performing metrics
def identify_low_performing_metrics(metric_ids, metric_data):
    metric_values = {metric: metric_data[metric].mean() for metric in metric_ids if metric in metric_data.columns}
...
def identify_low_performing_groups(group_ids, metric_data):
    group_values = {group: metric_data[group].mean() for group in group_ids if group in metric_data.columns}
...
def identify_low_performing_sub_modules(sub_module_ids, metric_data):
    sub_module_values = {sub_module: metric_data[sub_module].mean() for sub_module in sub_module_ids if sub_module in metric_data.columns}
...
def print_and_save_performance_averages(low_performing_metrics, low_performing_values, low_performing_groups, low_performing_group_values, low_performing_sub_modules, low_performing_sub_module_values, module_averages, json_data, output_report_file):
    ...
    print(f"\nOverall Module Average (BSPT_001): {module_average:.2f}")
...
output_report_file = 'low_performing_entities_report_bspt.txt'
...
##Module_values_bspt.csv retained as the output file.
##Filename says 2_0 (Praveen's upload), contents are 3_0-parity: this was branched
##from an older base than the Customer/Shared 3_0, so the 'value' in sub_module
##guard was ported in. Kept the 2_0 filename so provenance to Repo-01 stays clear.
