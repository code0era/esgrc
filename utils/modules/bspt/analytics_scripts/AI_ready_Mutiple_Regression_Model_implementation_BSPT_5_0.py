...
# -------------------------------
file_path = 'module_values_bspt.csv'
...
if duplicate_cols:
    ...
    raise ValueError("Duplicate column IDs found in module_values_bspt.csv. Please fix before analysis.")
else:
...
with open("bspt_performance_json_file.json", "r") as f:
...
# -------------------------------
X = metrics_data.drop(columns=['BSPT_001'])
y = metrics_data['BSPT_001']
input_dim = X.shape[1]
...
baseline_summary = (
    f"Baseline Predictor (Mean of BSPT_001):\n"
    f"MSE: {baseline_mse}\n"
...
with open("BSPT_Module_model_summary.txt", "w") as f:
    f.write("\n".join(report_lines))
    ...
    f.write("\n")
    f.write("----------------------------------End of Business Partner Module Risk Analysis Summary---------------------------\n")
print("Full analysis written to BSPT_Module_model_summary.txt")
