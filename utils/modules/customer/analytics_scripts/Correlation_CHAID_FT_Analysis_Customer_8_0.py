...
# -----------------------------
def build_metric_lookup(json_file_path='customer_performance_json_file.json'):
    """
...
# dump. Customer has 348 metrics (vs ESGRC's 84), so the full pairwise outputs
# scale ~17x larger - the correlation matrix dump and inconsistency table
# together drove the combined step-6 report to 9.1MB and overflowed Claude's
# context window on 2026-07-31 (guard.py's fallback char/token estimate also
# undercounted the resulting dense tabular content - see guard.py's
# CHARS_PER_TOKEN_FALLBACK comment for that half of the fix). This function
...
def main(metrics_csv_file_path, group_csv_file_path, sub_module_csv_file_path):
    ...
    json_path = 'customer_performance_json_file.json'  # Adjust path if needed
    ...
    # File names
    output_report_file = 'M_G_SM_correlation_report_customer.txt'
    trends_file = 'trends_and_repetitions_report_customer.txt'
    inconsistencies_file = 'inconsistencies_report_customer.txt'
    chaid_report_file = 'chaid_risk_segmentation_report_customer.txt'
...
if __name__ == "__main__":
    metrics_csv_file_path = 'filtered_metrics_data_customer.csv'
    group_csv_file_path = 'filtered_groups_data_customer.csv'
    sub_module_csv_file_path = 'filtered_sub_modules_data_customer.csv'
