...
# -------------------------------
file_path = 'module_values_customer.csv'
...
if duplicate_cols:
    ...
    raise ValueError("Duplicate column IDs found in module_values_customer.csv. Please fix before analysis.")
else:
...
with open("customer_performance_json_file.json", "r") as f:
...
# -------------------------------
X = metrics_data.drop(columns=['CUST_001'])
y = metrics_data['CUST_001']
input_dim = X.shape[1]
...
baseline_summary = (
    f"Baseline Predictor (Mean of CUST_001):\n"
    f"MSE: {baseline_mse}\n"
...
with open("Customer_Module_model_summary.txt", "w") as f:
    f.write("\n".join(report_lines))
    ...
    f.write("\n")
    f.write("----------------------------------End of Customer Module Risk Analysis Summary---------------------------\n")
print("Full analysis written to Customer_Module_model_summary.txt")
