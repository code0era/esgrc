import sys
try:  # keep emoji/non-ASCII prints from crashing on Windows (cp1252) consoles
...
# Seeded so identical input yields identical scenarios; these feed a ranking
# that otherwise changed on every run of the same data. The draws remain
# synthetic noise unrelated to the input, so the ranking is not evidence about
# the business - a methodology question for the analytics owner, which a seed
# does not answer.
np.random.seed(int(os.environ.get("ANALYTICS_SEED", "42")))
...
# The keys above are metric-level IDs (INM10101 etc.) but compute_overall_risk
# iterates sub-module columns of the form XXX10000, so not one of them ever
# matched: every weight fell back to 1.0 and the overall risk was a plain
# unweighted mean, while the report below announced 'Weights applied in this
# run' and listed all three. Split matched from unmatched so the number and the
# report agree, and say so loudly when a configured weight does nothing.
_sub_col_set = set(sub_cols)
applied_weights = {k: v for k, v in custom_weights.items() if k in _sub_col_set}
unmatched_weights = {
    k: v for k, v in custom_weights.items() if k not in _sub_col_set
}
if unmatched_weights:
    print(
        f"WARNING: {len(unmatched_weights)} configured weight(s) match no "
        f"sub-module column and were NOT applied: {sorted(unmatched_weights)}"
    )

# --- Compute Overall Risk ---
overall_risk = compute_overall_risk(df, sub_cols, weights=applied_weights)
...
with open(report_path, "w") as f:
    ...
    if not applied_weights:
        f.write("- none: no configured weight matched a sub-module column, so "
                "this run is an unweighted mean\n")
    for m in unmatched_weights:
        f.write(f"- {m}: NOT APPLIED (no matching sub-module column)\n")
    for m, w in applied_weights.items():
