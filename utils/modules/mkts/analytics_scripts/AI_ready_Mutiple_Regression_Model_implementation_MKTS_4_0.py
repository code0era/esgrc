...
# -------------------------------
file_path = 'module_values_mkts.csv'
...
if duplicate_cols:
    ...
    raise ValueError("Duplicate column IDs found in module_values_mkts.csv. Please fix before analysis.")
else:
...
with open("mkts_performance_json_file.json", "r") as f:
...
# -------------------------------
X = metrics_data.drop(columns=['MKTS_001'])
y = metrics_data['MKTS_001']
input_dim = X.shape[1]
...
baseline_summary = (
    f"Baseline Predictor (Mean of MKTS_001):\n"
    f"MSE: {baseline_mse}\n"
...
with open("MKTS_Module_model_summary.txt", "w") as f:
    f.write("\n".join(report_lines))
    ...
    f.write("\n")
    f.write("----------------------------------End of MKTS Module Risk Analysis Summary---------------------------\n")
print("Full analysis written to MKTS_Module_model_summary.txt")
