...
# -------------------------------
file_path = 'module_values_shared.csv'
...
if duplicate_cols:
    ...
    raise ValueError("Duplicate column IDs found in module_values_shared.csv. Please fix before analysis.")
else:
...
with open("shared_performance_json_file.json", "r") as f:
...
# -------------------------------
X = metrics_data.drop(columns=['SHRD_001'])
y = metrics_data['SHRD_001']
input_dim = X.shape[1]
...
baseline_summary = (
    f"Baseline Predictor (Mean of SHRD_001):\n"
    f"MSE: {baseline_mse}\n"
...
with open("Shared_Module_model_summary.txt", "w") as f:
    f.write("\n".join(report_lines))
    ...
    f.write("\n")
    f.write("----------------------------------End of Shared Module Risk Analysis Summary---------------------------\n")
print("Full analysis written to Shared_Module_model_summary.txt")
