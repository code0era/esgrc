...
# -------------------------------
file_path = 'module_values_resource.csv'
...
if duplicate_cols:
    ...
    raise ValueError("Duplicate column IDs found in module_values_resource.csv. Please fix before analysis.")
else:
...
with open("resource_performance_json_file.json", "r") as f:
...
# -------------------------------
X = metrics_data.drop(columns=['RSRC_001'])
y = metrics_data['RSRC_001']
input_dim = X.shape[1]
...
baseline_summary = (
    f"Baseline Predictor (Mean of RSRC_001):\n"
    f"MSE: {baseline_mse}\n"
...
with open("Resource_Module_model_summary.txt", "w") as f:
    f.write("\n".join(report_lines))
    ...
    f.write("\n")
    f.write("----------------------------------End of Resource Module Risk Analysis Summary---------------------------\n")
print("Full analysis written to Resource_Module_model_summary.txt")
