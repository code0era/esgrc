...
# -------------------------------
file_path = 'module_values_integration.csv'
...
if duplicate_cols:
    ...
    raise ValueError("Duplicate column IDs found in module_values_integration.csv. Please fix before analysis.")
else:
...
with open("integration_performance_json_file.json", "r") as f:
...
# -------------------------------
# Seeded so a given input always produces the same scenarios. These draws feed
# the "metrics driving risk" ranking further down, which meant that ranking - a
# client-facing output - changed on every run of identical data. Seeding makes
# the result reproducible and auditable; override to explore other draws.
# NOTE: the scenarios remain synthetic noise unrelated to the input data, so the
# driver ranking derived from them is not evidence about the business. That is a
# methodology question for the analytics owner, not something a seed fixes.
...
# -------------------------------
X = metrics_data.drop(columns=['INTG_001'])
y = metrics_data['INTG_001']
input_dim = X.shape[1]
...
baseline_summary = (
    f"Baseline Predictor (Mean of INTG_001):\n"
    f"MSE: {baseline_mse}\n"
...
with open("Integration_Module_model_summary.txt", "w") as f:
    f.write("\n".join(report_lines))
    ...
    f.write("\n")
    f.write("----------------------------------End of Integration Module Risk Analysis Summary---------------------------\n")
print("Full analysis written to Integration_Module_model_summary.txt")

if __name__ == '__main__':
    pass  # app.run(debug=True) disabled - Flask dev server blocked batch runs (never exits). All outputs are written at module level above.
