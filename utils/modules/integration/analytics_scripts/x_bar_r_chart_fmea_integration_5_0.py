...
# ---------- Report Generators ----------
# Columns shown in the RPN summary - shared by the .txt (Claude input) and the
# PDF (stakeholder download) so both render exactly the same table.
RPN_SUMMARY_COLS = ["metric_id", "mean", "sigma", "UCL", "LCL", "signals", "RPN", "analysis_date"]

def save_rpn_summary_txt(summary: pd.DataFrame, filename: str):
    # Text mirror of the RPN summary PDF (same columns/order). This is spec
    # report #7 ("RPN summary") - it feeds the Step 6 combine and reaches Claude,
    # while the PDF below remains the user-download stakeholder view.
    summary[RPN_SUMMARY_COLS].to_csv(filename, sep="\t", index=False)

def save_rpn_summary_pdf(summary: pd.DataFrame, filename: str, rows_per_page=30):
    cols = RPN_SUMMARY_COLS
...
if __name__ == "__main__":
    df_wide = pd.read_csv("input_metric_values_integration.csv", sep=",")
    long_df, summary, metric_ids = analyze_wide(df_wide)

    # Save SPC/Six-Sigma metrics summary TXT (spec report #6)
    summary.to_csv(f"metrics_summary_integration_{ANALYSIS_DATE}.txt", sep="\t", index=False)

    # Save RPN summary as TXT (spec report #7 - fed to Claude) and as PDF (stakeholder download)
    save_rpn_summary_txt(summary, f"rpn_summary_integration_{ANALYSIS_DATE}.txt")
    save_rpn_summary_pdf(summary, f"RPN_summary_report_integration_{ANALYSIS_DATE}.pdf")

    # Save SPC charts PDF
    save_spc_charts_pdf(long_df, metric_ids, f"SPC_charts_report_integration_{ANALYSIS_DATE}.pdf")
