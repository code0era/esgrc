...
# -------------------------------
file_path = 'module_values_service.csv'
...
if duplicate_cols:
    ...
    raise ValueError("Duplicate column IDs found in module_values_service.csv. Please fix before analysis.")
else:
...
with open("service_performance_json_file.json", "r") as f:
...
# -------------------------------
X = metrics_data.drop(columns=['SRVC_001'])
y = metrics_data['SRVC_001']
input_dim = X.shape[1]
...
baseline_summary = (
    f"Baseline Predictor (Mean of SRVC_001):\n"
    f"MSE: {baseline_mse}\n"
...
with open("Service_Module_model_summary.txt", "w") as f:
    f.write("\n".join(report_lines))
    ...
    f.write("\n")
    f.write("----------------------------------End of Service Module Risk Analysis Summary---------------------------\n")
print("Full analysis written to Service_Module_model_summary.txt")
