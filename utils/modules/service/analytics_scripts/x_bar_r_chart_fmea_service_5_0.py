...
if __name__ == "__main__":
    df_wide = pd.read_csv("input_metric_values_service.csv", sep=",")
    long_df, summary, metric_ids = analyze_wide(df_wide)

    # Save SPC/Six-Sigma metrics summary TXT (spec report #6)
    summary.to_csv(f"metrics_summary_service_{ANALYSIS_DATE}.txt", sep="\t", index=False)

    # Save RPN summary as TXT (spec report #7 - fed to Claude) and as PDF (stakeholder download)
    save_rpn_summary_txt(summary, f"rpn_summary_service_{ANALYSIS_DATE}.txt")
    save_rpn_summary_pdf(summary, f"RPN_summary_report_service_{ANALYSIS_DATE}.pdf")

    # Save SPC charts PDF
    save_spc_charts_pdf(long_df, metric_ids, f"SPC_charts_report_service_{ANALYSIS_DATE}.pdf")
