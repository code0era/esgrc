...
# -------------------------------
file_path = 'module_values_ictm.csv'
...
if duplicate_cols:
    ...
    raise ValueError("Duplicate column IDs found in module_values_ictm.csv. Please fix before analysis.")
else:
...
with open("ictm_performance_json_file.json", "r") as f:
...
# -------------------------------
X = metrics_data.drop(columns=['PRCY_001'])
y = metrics_data['PRCY_001']
input_dim = X.shape[1]
...
baseline_summary = (
    f"Baseline Predictor (Mean of PRCY_001):\n"
    f"MSE: {baseline_mse}\n"
...
with open("ICTM_Module_model_summary.txt", "w") as f:
    f.write("\n".join(report_lines))
    ...
    f.write("\n")
    f.write("----------------------------------End of ICTM Module Risk Analysis Summary---------------------------\n")
print("Full analysis written to ICTM_Module_model_summary.txt")
